local MAJOR, MINOR = "LibDateTimePicker-1.0", 1
...
function DateTimePicker:UpdateCalendarButtonIcons()
  if not self.calendarButtons then return end
  ...
  for b in pairs(self.calendarButtons) do
...
function DateTimePicker:Ensure()
  ...
  local pickerFrame = CreateFrame("Frame", nil, UIParent, "BackdropTemplate")
  ...
  local function StyleHeaderChevron(btn, atlas, fallbackText)
    ...
    btn.fallbackText = fallbackText or btn.fallbackText or ""
    btn:SetText("")
    local fontString = btn:GetFontString()
    if fontString then fontString:Hide() end
    ...
    local highlightTexture = btn.hlTex or btn:CreateTexture(nil, "HIGHLIGHT")
    highlightTexture:SetPoint("CENTER")
    highlightTexture:SetAtlas(atlas, true)
    highlightTexture:SetSize(14, 14)
    highlightTexture:SetBlendMode("ADD")
    highlightTexture:Show()
    btn.hlTex = highlightTexture
    btn:SetHighlightTexture(highlightTexture)
  end
  ...
  for r = 1, 6 do
    for c = 1, 7 do
      ...
      dayButton:SetScript("OnClick", function()
        ...
        if dayButton.year and dayButton.month and dayButton.day then
          state.year, state.month, state.day = dayButton.year, dayButton.month, dayButton.day
  ...
  pickerFrame.SetTimeUI = setTimeUI
...
function DateTimePicker:RefreshCalendar()
  ...
  for i = 1, 42 do
    ...
    dayButton.year, dayButton.month, dayButton.day = cellYear, cellMonth, cellDay
...
function DateTimePicker:Open(editBox, order, isEnd, dateUtil)
  ...
  if pickerFrame.SetTimeUI then pickerFrame:SetTimeUI() end
...
function DateTimePicker:AttachCalendarButton(editBox, onClick)
  ...
  -- This should make the button collision proof.
  local existing = editBox[KEY_CALENDAR_BUTTON]
  ...
  editBox[KEY_CALENDAR_BUTTON] = calendarButton
  ...
  if not DateTimePicker.calendarButtons then
    DateTimePicker.calendarButtons = setmetatable({}, { __mode = "k" })
  end
  DateTimePicker.calendarButtons[calendarButton] = true

  DateTimePicker:UpdateCalendarButtonIcons()

  if not DateTimePicker.calendarTicker and C_Timer and C_Timer.NewTicker then
    DateTimePicker.calendarTicker = C_Timer.NewTicker(30, function()
      DateTimePicker:UpdateCalendarButtonIcons()
    end)
  end
