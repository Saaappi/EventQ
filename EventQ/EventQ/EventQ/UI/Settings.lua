...
function EventQWowStyle1DropdownControlMixin:InitDropdown()
  ...
  -- Blizzard's Settings.CreateDropdownOptionInserter expects a callable that returns the option table.
  -- If we pass the wrong arguments (or a raw table), Blizzard's menu build will throw a nil-call error.
  --
  -- In addition, both menu population and tooltip rendering call into the options provider.
  -- Wrap it defensively so an addon-side error cannot break the Settings panel.
  local optionsFunc
  if type(options) == "function" then
    optionsFunc = function()
      local ok, data = pcall(options)
    ...
    local optionTable = options
    optionsFunc = function()
      return type(optionTable) == "table" and optionTable or {}
    end
  end

  -- Use the same safe options provider for both tooltip and menu insertion.
